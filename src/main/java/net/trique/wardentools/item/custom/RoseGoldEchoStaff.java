...
public class RoseGoldEchoStaff extends Item {

    public RoseGoldEchoStaff(Settings settings) {
    ...
    public void usageTick(World world, LivingEntity user, ItemStack stack, int remainingUseTicks) {
        ...
        if(getMaxUseTime(stack, user) - remainingUseTicks == 1) {
            world.playSound(null, user.getX(), user.getY(), user.getZ(), SoundEvents.ENTITY_WARDEN_SONIC_CHARGE, SoundCategory.BLOCKS, 2.0f, 1.0f);
        }
    }
    ...
    private void spawnSonicBoom(World world, LivingEntity user) {
        ...
        world.playSound(null, user.getX(), user.getY(), user.getZ(), SoundEvents.BLOCK_TRIAL_SPAWNER_EJECT_ITEM, SoundCategory.BLOCKS, 4.0f, 1.0f);
        ...
        for (int particleIndex = 1; particleIndex < MathHelper.floor(offsetToTarget.length()) + 14; ++particleIndex) {
            Vec3d particlePos = source.add(normalized.multiply(particleIndex));
            ((ServerWorld) world).spawnParticles(WardenParticles.ROSE_GOLD_SONIC_BOOM, particlePos.x, particlePos.y, particlePos.z, 1, 0.0, 0.0, 0.0, 0.0);
        ...
        for (Entity hitTarget : hit) {
            if(hitTarget instanceof LivingEntity living) {
                living.damage(world.getDamageSources().sonicBoom(user), 20.0f);
